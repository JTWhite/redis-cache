...
class RedisCache:
    ...
    def _construct_key(func: Callable, args: tuple[Any, ...], kwargs: dict[str, Any], key_prefix: str) -> str:
        ...
        key_vars = {varnames[i]: args[i] for i in range(len(args)) if i < len(args)} | kwargs
    ...
    @classmethod
    def cache(cls, client: RedisClient=RedisClient(), timeout: int=60, key_prefix: str='', serialize_encoder: str='json'):
        def decorator(func):
            def wrapper(*args, **kwargs):
                ...
                return cls._get_result(client, key, serializer) if client.exists(key) else cls._set_result(client, key, serializer, func, args, kwargs)
